...
public class DeleteStudent extends JFrame implements ActionListener
{
	...
	public DeleteStudent()
	{
		...
		p = new JPanel();
		setTitle("Do you want to Delete Student ");
		setLocation(600,200);
		setSize(700,700);
		...
		L9.setBounds(150,490,100,20);
		L9.setFont(new Font("Arial",Font.PLAIN,17));
		
		T9 = new JLabel();
		T9.setBounds(290,490,200,20);
		T9.setFont(f);
		p.add(L9); 		p.add(T9);
		
		L10 = new JLabel("Paid Amt :");
		L10.setBounds(150,540,100,20);
		L10.setFont(new Font("Arial",Font.PLAIN,17));
		
		T10 = new JLabel();
		T10.setBounds(290,540,200,20);
		T10.setFont(f);
		p.add(L10); 		p.add(T10);
		...
		public void itemStateChanged(ItemEvent Aobj){
			try
			{
				...
				Connection Cobj = DriverManager.getConnection("jdbc:mysql://localhost/Coaching_Institute","root"," ");
				Statement sobj = Cobj.createStatement();
				ResultSet rsobj = sobj.executeQuery("select * from Student where Id= "+ch.getSelectedItem()+";");
				
					while(rsobj.next())
					{	
						T2.setText(rsobj.getString("FName")); 
						T3.setText(rsobj.getString("LName")); 
						T4.setText(rsobj.getString("Gender")); 
						T5.setText(rsobj.getString("ContactNo")); 
						T6.setText(rsobj.getString("CName")); 
						T7.setText(rsobj.getString("FcltName")); 
						T8.setText(rsobj.getString("DOD")); 
						T9.setText(rsobj.getString("totalAmt")); 
						T10.setText(rsobj.getString("paidAmt")); 
					}
				}catch(Exception e){	e.printStackTrace();	}
			}
		});
	...
	public void del()
	{
		try
		{
			...
			Connection Cobj = DriverManager.getConnection("jdbc:mysql://localhost/Coaching_Institute","root"," ");
			Statement sobj = Cobj.createStatement();
			String insertQry = "delete from Student where Id=" + ch.getSelectedItem()+";";
			int count = sobj.executeUpdate(insertQry);
			System.out.print(insertQry);
			
			
			String dltQry = "delete from Fee where StudentId=" + ch.getSelectedItem()+";";
